...
(
    ...
    `ACCESS_APP_ID` INT(11)     DEFAULT NULL COMMENT '访问者APPID',
    `SEQ_NO`        VARCHAR(32) DEFAULT NULL COMMENT '请求流水',
    `ASSET_API_ID`  INT(11)     DEFAULT NULL COMMENT '数据资产ID',
    ...
    `IP`            VARCHAR(20) DEFAULT NULL COMMENT '调用IP',
...
(
    ...
    `RECORD_DATA`      INT(11)  DEFAULT NULL COMMENT '统计的数据',
    `RECORD_ENTITY_ID` INT(11)  DEFAULT NULL COMMENT '统计数据主体ID',
    `DATA_TIME`        DATETIME DEFAULT NULL COMMENT '创建时间',
    `STAT_TIME`        DATETIME DEFAULT NULL COMMENT '统计时间',
    `RESULT`           MEDIUMTEXT COMMENT '统计结果数量',
    PRIMARY KEY (`ID`),
...
(
  ...
  DEFAULT CHARSET = utf8 COMMENT ='API访问参数';
...
(
    ...
    `DATA_ASSET_ID`          int(11)       DEFAULT NULL COMMENT '数据资产ID',
    `ACCESS_APP_ID`          int(11)       DEFAULT NULL COMMENT '使用者ID',
    `FORM_URL`               varchar(1024) DEFAULT NULL COMMENT 'K2表单地址',
...
(
    ...
    `DATA_ACCESS_KEY`         varchar(255) comment '接入key',
    `DATA_ACCESS_SECRET`      varchar(255)  COMMENT '数据访问密钥',
    `DATA_ACCESS_DESCRIPTION` varchar(255) COMMENT '接入端描榜',
    `UPDATE_TIME`             datetime     NOT NULL,
    `CREATE_TIME`             datetime     NOT NULL,
...
ALTER TABLE `DW_OPEN_APP_ACCESS`
    ADD COLUMN `STATUS`  int NULL DEFAULT 0 COMMENT '状态' AFTER `USERNAME`,
    ADD COLUMN `APPROVAL_MESSAGE`  varchar(255) NULL DEFAULT '' COMMENT '审批意见' AFTER `STATUS`;
ALTER TABLE `DW_OPEN_APP_ACCESS`
    MODIFY COLUMN `DATA_ACCESS_KEY`  varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL AFTER `DATA_ACCESS_APP_NAME`,
    MODIFY COLUMN `DATA_ACCESS_SECRET`  varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL COMMENT '数据访问密钥' AFTER `DATA_ACCESS_KEY`;
...
(
    ...
    `DATA_ASSET_NAME`           varchar(255)          DEFAULT NULL COMMENT '数据资产名称',
    `DATA_ASSET_DESCRIPTION`    varchar(255)          DEFAULT '' COMMENT '数据资产描述',
    ...
    `DATA_ASSET_API_METHOD`     varchar(255)          DEFAULT NULL COMMENT '数据资产API名称',
    `UPDATE_TIME`               datetime     NOT NULL,
    `CREATE_TIME`               datetime     NOT NULL,
    `ICON_URL`                  varchar(512)          DEFAULT NULL COMMENT '图标路径',
    `API_NAME`                  varchar(32)  NOT NULL COMMENT 'API名称',
    `UPDATE_FREQUENCY`          int(11)      NOT NULL COMMENT '更新频率',
    `PROTOCOL`                  int(11)      NOT NULL COMMENT '请求协议',
    `SECRET`                    tinyint(4)            DEFAULT NULL COMMENT '加密方式',
    `API_GROUP_ID`              int(11)      NOT NULL COMMENT '分组ID',
    `API_DESCRIPTION`           varchar(1024)         DEFAULT NULL COMMENT 'API描述',
    `RESPONSE_CONTENT_TYPE`     varchar(255) NOT NULL COMMENT '返回格式',
    `REQ_METHOD`                varchar(255) NOT NULL DEFAULT '' COMMENT '请求方法',
    `DATASOURCE_CONFIG_ID`      int(11)               DEFAULT NULL COMMENT '数据源配置ID',
    `PUBLISHER`                 varchar(64) COMMENT '发布者',
...
(
    `ID`              int(11) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
    `CONNECTION_NAME` varchar(32)  DEFAULT NULL COMMENT '连接名称',
    `DB_TYPE`         int(11)      DEFAULT NULL COMMENT '数据库类型ID',
    `DB_NAME`         varchar(64)  DEFAULT NULL COMMENT '数据库名称',
    `DB_HOST`         varchar(256) DEFAULT NULL COMMENT '数据库地址',
    `DB_PORT`         int(11)      DEFAULT NULL COMMENT '数据库端口',
    `DB_USERNAME`     varchar(64)  DEFAULT NULL COMMENT '数据库用户名',
    `DB_PASSWORD`     varchar(64)  DEFAULT NULL COMMENT '数据库密码',
    `ZK_NODE`         varchar(64)  DEFAULT NULL COMMENT 'ZOOKEEPER节点',
    `DB_DESCRIPTION`  varchar(256) DEFAULT NULL COMMENT '数据源描述',
    `CREATE_TIME`     datetime     DEFAULT NULL COMMENT '创建时间',
    `UPDATE_TIME`     datetime     DEFAULT NULL COMMENT '更新时间',
    `PARENT_ID`       int(11)      DEFAULT NULL COMMENT '父配置ID',
    `CHILDREN_ID`     int(11)      DEFAULT NULL COMMENT '子配置ID',
    PRIMARY KEY (`ID`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
(
    `ID`                int(11)     NOT NULL AUTO_INCREMENT COMMENT '自增ID',
    `DATABASE_NAME`     varchar(32)  DEFAULT NULL COMMENT '数据库名称',
    `DRIVER_CLASS_NAME` varchar(128) DEFAULT NULL COMMENT '驱动类名',
    `CREATE_TIME`       datetime     DEFAULT NULL COMMENT '创建时间',
    `UPDATE_TIME`       datetime     DEFAULT NULL COMMENT '更新时间',
    `PIC_URL`           varchar(128) DEFAULT NULL COMMENT '图片地址',
    `DEFAULT_PORT`      varchar(32) NULL COMMENT '默认端口',
    PRIMARY KEY (`ID`)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
(
    ...
    `GROUP_PATH`  VARCHAR(128) COMMENT '分组路径',
    `GROUP_DESC`  VARCHAR(128) COMMENT '分组描述',
    `CREATE_TIME` DATETIME DEFAULT NULL COMMENT '创建时间',
    `UPDATE_TIME` DATETIME DEFAULT NULL COMMENT '更新时间'
) ENGINE = INNODB;
...
(
  ...
  DEFAULT CHARSET = utf8;
CREATE TABLE DW_OPEN_USER_LOGIN
(
    ID          INT PRIMARY KEY AUTO_INCREMENT COMMENT '自增ID',
    USERNAME    VARCHAR(32) COMMENT '用户名',
    CREATE_TIME VARCHAR(32) COMMENT '登陆时间',
