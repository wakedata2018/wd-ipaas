...
public class DateFunc implements IFunc {
	...
	@Override
	public void init() {
        FunctionEnumSet.DateEnum[] dateEnums = FunctionEnumSet.DateEnum.values();
        Arrays.stream(dateEnums).forEach(x-> FuncExecutor.register(x.method(), this));
	}
	...
	private Date parse(String dateStr, String pattern, String... timeZone) {
		...
		try {
			...
			throw new OpenException(OpenApiMsgCodeEnum.w_func_exception_msg.getCode(),"Parse date error, dateStr=" + dateStr + " pattern=" + pattern, e);
		}
	}
