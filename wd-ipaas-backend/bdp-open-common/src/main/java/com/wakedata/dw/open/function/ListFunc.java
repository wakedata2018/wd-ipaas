...
public class ListFunc implements IFunc {
	...
	@Override
	public void init() {
		FunctionEnumSet.ListEnum[] listEnums = FunctionEnumSet.ListEnum.values();
		Arrays.stream(listEnums).forEach(x-> FuncExecutor.register(x.method(), this));
	}
		...
		for (Map<String, Object> m : data) {
			for (String fieldPair : fieldPairs) {
				...
				if (parts == null || parts.length != 2) {
					...
					throw new OpenException(OpenApiMsgCodeEnum.w_func_exception_msg.getCode(),
	 ...
	 * @return List<Object>
	 */
	public List<Object> convertToList(Object... objects) {
	 ...
	 * @return
	 */
	public List<String> stringToList(String string,String splitRegex){
		if(StrUtil.isBlank(string)){
			return null;
		}
		if(StrUtil.isBlank(splitRegex)){
			return Collections.singletonList(string);
		}
		return Arrays.stream(string.split(splitRegex)).collect(Collectors.toList());
	}
