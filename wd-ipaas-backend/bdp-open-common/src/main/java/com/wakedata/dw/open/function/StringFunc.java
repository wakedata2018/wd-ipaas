...
public class StringFunc implements IFunc {
	...
	@Override
	public void init() {
		FunctionEnumSet.StringEnum[] stringEnums = FunctionEnumSet.StringEnum.values();
		Arrays.stream(stringEnums).forEach(x-> FuncExecutor.register(x.method(), this));
	}
	...
	public String substring(String str, int beginIndex, int... endIndex) {
		...
		if (endIndex != null && endIndex.length > 0) {
			if (endIndex.length > 1) {
				...
				throw new OpenException(OpenApiMsgCodeEnum.w_func_exception_msg.getCode(),"invalid argument: endIndex");
			}
			if(endIndex[0] == -2){
