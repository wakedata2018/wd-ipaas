...
package com.wakedata.dw.open.input;
...
public class PathMapping {
	...
	public static ONode transform(ONode ctxNode, Map<String, Object> rules, boolean supportMultiLevels) {
		...
		for (Entry<String, Object> entry : rs.entrySet()) {
			if (entry.getValue() instanceof String) {
				...
				if (WILDCARD_STAR.equals(entry.getKey())) {
					starValObj = obj;
	...
	private static Object getRefValue(ONode ctxNode, String type, String path) {
		...
		if (path.startsWith(IFunc.NAME_SPACE_PREFIX)) {
			...
			try {
				...
				if (path.contains(IFunc.SEPARATOR_VERTICAL_BAR)) {
					p = path.substring(0, path.indexOf(IFunc.SEPARATOR_VERTICAL_BAR));
					defaultValue = path.substring(path.indexOf(IFunc.SEPARATOR_VERTICAL_BAR) + 1);
				}
				...
				throw new OpenException(String.format("path mapping errer: %s , path mapping data: %s %s", e.getMessage(), type, path), e);
			}
		}
		return obj;
	}
	...
	private static Object cast(Object obj, String type, String path) {
		try {
			...
			// e.printStackTrace();
			throw new OpenException(String.format("failed to cast %s to %s, JSON path expression: %s, error: %s", obj, type, path, e.getMessage()), e);
		}
	}
	...
	public static ONode select(ONode ctxNode, String path) {
		if(path.startsWith(IFunc.PRE_SPLIT)){
			return ctxNode.select(path);
		}
