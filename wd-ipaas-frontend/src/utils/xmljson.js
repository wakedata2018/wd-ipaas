...
XML.ObjTree.prototype.parseXML = function (xml, filterRoot) {
  let root;
  let xmldom;
  if (window.DOMParser) {
  ...
  return this.parseDOM(root, filterRoot);
};
...
XML.ObjTree.prototype.parseHTTP = function (url, options, callback) {
  ...
  if (typeof HTTP !== 'undefined' && HTTP.Request) {
    myopt.uri = url;
    const req = new HTTP.Request(myopt); // JSAN
    if (req) {trans = req.transport;}
  } else if (typeof Ajax !== 'undefined' && Ajax.Request) {
    const req = new Ajax.Request(url, myopt); // ptorotype.js
    if (req) {trans = req.transport;}
  }
...
XML.ObjTree.prototype.parseDOM = function (root, filterRoot) {
  if (!root) {
    return;
  }
  ...
  if (root.nodeType !== 11 && !filterRoot) {
...
XML.ObjTree.prototype.hash_to_xml = function (name, tree) {
  ...
  if (typeof name === 'undefined' || name == null) {
    ...
    if (jelem.match(/\n/)) {
      ...
      jelem = '<' + tagName + jattr + '>' + jelem + '</' + tagName + '>\n';
    }
  } else {
    jelem = '<' + name + jattr + ' />\n';
  }
...
XML.ObjTree.prototype.xml_unescape = function (text) {
  return text ? (reEscapedHtml.test(text) ? text.replace(reEscapedHtml, entity => unescapes[entity]) : text) : '';
};
