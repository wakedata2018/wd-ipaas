<template>
  <el-form ref="formRef" :model="form" class="api-params-table">
    <el-table :data="value" style="width: 100%" class="dss-table bd-table" :row-key="returnRowKey">
      <el-table-column v-if="!isHidden('assetColumns')" prop="assetColumns" label="参数名称" max-width="100">
        <template #default="scope">
          ...
          <el-form-item :prop="'list.' + scope.$index + '.assetColumns'" :rules="rules.assetColumns">
            <el-input
      ...
      <el-table-column v-if="!isHidden(defaultParamMap.dataType)" :prop="defaultParamMap.dataType" label="参数类型">
        <template #default="scope">
          <el-form-item :prop="'list.' + scope.$index + '.' + [defaultParamMap.dataType]">
            <el-select
      ...
      <el-table-column v-if="!isHidden(defaultParamMap.sample)" :prop="defaultParamMap.sample" label="示例值">
        <template #default="scope">
          <el-form-item style="width: 100%" :prop="'list.' + scope.$index + '.' + [defaultParamMap.sample]">
            <el-input
      ...
      <!-- 选择方式 -->
      <el-table-column v-if="isFlow || isTimerTask" prop="paramValueType" label="选择方式" max-width="100px">
        <template #default="scope">
          <el-form-item style="margin: 0px; height: auto" :prop="'list.' + scope.$index + '.paramValueType'">
            <el-select
      ...
      <!-- 参数值 -->
      <el-table-column v-if="isFlow || isTimerTask" prop="expression" label="参数值">
        <template #default="scope">
          <el-form-item :prop="'list.' + scope.$index + '.expression'">
            ...
            <div v-else-if="scope.row.paramValueType === ModeEnum.reference" class="params">
              <div v-if="scope.row.expression" class="map-value">
                <el-tooltip :content="scope.row.expression" placement="top">
                  <el-input v-model.trim="scope.row.expression" placeholder="请输入表达式"></el-input>
                </el-tooltip>
                <i class="el-icon-setting expression-setting" @click="selectParams(scope.row)"></i>
              </div>
              <el-button v-else type="text" class="select-params" @click="selectParams(scope.row)">选择参数 </el-button>
            </div>
            <div v-else-if="scope.row.paramValueType === ModeEnum.method">
              ...
              <div v-else class="expression-btn">
                <el-button type="text" class="select-params" @click="selectMethod(scope.row)">配置表达式</el-button>
              </div>
            </div>
            <!-- 固定值 -->
            <el-input
            ...
            <el-input
              ...
              type="text"
              :disabled="disableSelectParams(scope.row)"
            ></el-input>
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column v-if="!isHidden(defaultParamMap.description)" :prop="defaultParamMap.description" label="描述">
        <template #default="scope">
          <el-form-item :prop="'list.' + scope.$index + '.' + [defaultParamMap.description]" style="width: 100%">
            <el-input
...
<script>
  ...
  export default {
      ...
      return {
        ...
        rules: {
          assetColumns: [
            {
              required: true,
              validator: Validator.httpFieldEnNameValidator,
              trigger: 'blur',
            },
          ],
        },
    ...
    methods: {
      returnRowKey(row, data) {
        // 如果 assetColumns 这列是输入框，设置成row-key会导致输入一个字符就失去焦点
        return !this.isColFixed('assetColumns') ? Math.random() : `${row.assetColumns}`;
      },
