        ...
        <el-form label-width="120px" style="margin-top: 8px">
          <el-row :gutter="20" class="assets-details-row">
            <el-col :span="12" v-if="apiInfo && (apiInfo.dataAssetName || apiInfo.apiSql)">
              <el-form-item label="资产名称：" show-overflow-tooltip><div
                  class="name-title"
                  :title="
            ...
            <el-col :span="12">
              <el-form-item label="API Path："
                >/{{ apiInfo && apiInfo.dataAssetApiMethod }}</el-form-item
              >
            </el-col>
          </el-row>
...
export default {
  ...
  computed: {
    ...
    appInfo() {
      return (
        this.appList.find((app) => app.dataAccessAppId === this.appId) || {}
      );
    },
    finalParams() {
      ...
      const params = {
        ...
        dataAccessRuleFieldList: checkeds.map(
          (col) => col.datasourceTableColumnName
        ),
      };
      let finalParams = [
        ...(checkeds.length > 0 ? [params] : []),
      ];
      return finalParams;
    }
  },
    ...
    getAssetTable() {
      ...
      ruleAPI
        ...
        .done((res) => {
          ...
          this.assetTable = this.addApplyProp(
            res.data.dataAccessRuleDetailList
          );
        })
        .always(() => {
          this.loading = false;
