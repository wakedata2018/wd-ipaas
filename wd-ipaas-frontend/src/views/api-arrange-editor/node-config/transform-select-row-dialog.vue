  ...
  >
    <el-scrollbar class="scrollbar">
      <el-form ref="form" :model="form" :rules="rules" label-position="left" label-width="120px" @submit.native.prevent>
        ...
        <el-form-item label="行表达式" prop="rowExpression">
          <template slot="label">
            <tips-icon v-if="templateStr" :content="'例：' + templateStr"></tips-icon>
            行表达式
          </template>
          <el-input
...
<script>
  ...
  import { getInitForm } from '../node-convert/transform-select-row';
  ...
  export default {
    ...
    computed: {
      ...
      templateStr() {
        return this.curNode?.getModel?.().data?.template;
      },
    },
    watch: {
      isShow(val) {
    ...
    methods: {
      initForm() {
        ...
        // this.form = $.extend(true, {}, getInitForm(), this.config);
