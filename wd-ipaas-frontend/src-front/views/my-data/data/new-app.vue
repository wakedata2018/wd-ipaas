...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        user: state => state.user.loginName,
      }),
      isShow: {
        get() {
          return this.visible;
    ...
    watch: {
      ...
      selectedInCharge(val) {
        const item = this.inChargeList.find(inChargeItem => inChargeItem.loginName === val);
        if (item) {
          this.form.inCharge = [{ user: item.loginName, id: item.loginName }];
        }
      },
    },
    created() {
