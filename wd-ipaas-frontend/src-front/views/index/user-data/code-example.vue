    ...
    <el-tabs v-model="activeName" @tab-click="hanldeChange">
      ...
      <el-tab-pane label="返回数据" name="data">
        <el-table :data="cols" class="dss-table bd-table" v-loading="loading">
          <el-table-column prop="assetColumns" label="字段名称"></el-table-column>
          <el-table-column prop="assetDatatype" label="字段类型"></el-table-column>
          <el-table-column prop="descriptions" label="描述"></el-table-column>
        </el-table>
      </el-tab-pane>
    </el-tabs>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        this.dataAssetId = this.apiInfo.dataAssetApiId || null;
