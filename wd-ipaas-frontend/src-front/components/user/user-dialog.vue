      ...
      <el-col :span="multiple ? 12 : 24">
        <div class="transfer" v-loading="loading">
          ...
          <div class="transfer-body transfer-search transfer-page">
            ...
            <div class="content">
              <div v-for="user in userList" :key="user.zusrid" class="user-item">
                <el-checkbox
                ...
                <div
                  ...
                  :class="{'acitve-user': selected.findIndex(item => item.zusrid === user.zusrid) > -1}"
                >
                  <div class="user-tip">{{user.zorgname}}</div>
                  {{user.znachn}}
                  <span class="user-tip">（{{user.zusrid}}）</span>
      ...
      <el-col :span="12" v-if="multiple">
        <div class="transfer">
          ...
          <div class="transfer-body">
            <div class="content">
              <div v-for="user in selected" :key="user.zusrid" class="user-item">
...
export default {
  ...
  methods: {
    ...
    format(users) {
      users = users || [];
      users.forEach(user => {
        if (this.selected.findIndex(item => item.zusrid === user.zusrid) > -1) {
          user.checked = true;
        } else {
          user.checked = false;
        }
    ...
    onChange(checked, user) {
      if (checked) {
        ...
        const finded = this.userList.find(item => item.zusrid === user.zusrid);
        if (finded) {
          finded.checked = false;
        }
        this.selected = this.selected.filter(item => item.zusrid !== user.zusrid);
      }
    },
    onSearch() {
